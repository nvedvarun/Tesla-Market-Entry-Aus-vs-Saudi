# Tesla-Market-Entry
# Australia vs. Saudi Arabia Market Entry Case Study  
*Tesla FP&A / Business Operations Interview Project*

## Overview
This project evaluates Tesla’s market-entry potential in **Australia vs. Saudi Arabia**, based on EV readiness, regulation, infrastructure maturity, financial ROI, and long-term strategic value. It was completed for a Business Operations & FP&A case study.

**Deliverables included:**
- A structured market-entry strategy deck  
- An Excel financial model (TAM sizing, scenarios, profitability)  
- A scoring framework comparing both markets  

## Key Skills Used
- Financial modeling & TAM estimation  
- Market & policy analysis  
- KPI evaluation  
- Executive storytelling & slide synthesis  

## Key Finding
**Recommendation:** Enter **Australia** first.

Australia showed:
- Stronger EV adoption momentum  
- More mature charging & regulatory environment  
- Lower execution risk and faster path to profitability  

Saudi Arabia presents long-term potential but requires larger upfront investment and offers slower initial demand.

## Repository Contents
- 📄 Strategy Deck — `Ved_TeslaCaseStudy_vf.pdf`  
- 📊 Financial Model — Excel with scenarios & TAM  
- 📝 README — Project summary  

## What I Learned
- Structuring open-ended strategy problems  
- Building assumptions with limited data  
- Communicating insights clearly & visually  
